...
def softmax(x):
    ...
    tmp = tf.transpose(x) - tf.reduce_max(x,axis = 1)
    tmp0  = tf.reduce_sum(tf.exp(tmp),axis=0)
 
    out = tf.transpose(tf.divide(tf.exp(tmp),tmp0 ) ) 
...
def test_softmax_basic():
    ...
    array = np.array([[1001, 1002, 1003], [3, 4, 5]],dtype = np.float32)
    test3 = softmax(tf.constant(array, dtype=tf.float32))
    with tf.Session() as sess:
            test3 = sess.run(test3)
            test_all_close("Softmax test 3", test3, np.array([[ 0.09003057,  0.24472848,  0.66524094],[ 0.09003057,  0.24472848,  0.66524094]]))

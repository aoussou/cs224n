...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)        
        ...
        for i in range(n_words):
            ...
            # this case for when the window size if the same size or
            # larger than the sentence
            if (i <= window_size - 1) and (i >= n_words - window_size):
                tmp_list0 += start*window_size
                for word in sentence:
                    tmp_list0 += word 
                tmp_list0+=end*window_size            
            
            # beginning of the sentence, if the window is smaller than the sentence
            elif i <= window_size - 1:
                tmp_list0 += start*(window_size-i)                                  
                for word in sentence[:(i+window_size+1)]:
                    tmp_list0 += word             

            # end of the sentence, if the window is smaller than the sentence                
            elif i >= n_words - window_size:                
                for word in sentence[i-window_size:]:
                    tmp_list0 += word            
                tmp_list0+=end*(i - (n_words - window_size) +1)
                
            # middle of the sentence, if the window is smaller than the sentence
            else:
                for word in sentence[i-window_size:i+window_size+1]:
                    tmp_list0 += word   
              
            tmp_list = (tmp_list0,labels[i])

            windowed_data.append(tmp_list)
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embed_index,(-1,self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(weight_initializer((self.config.n_window_features * self.config.embed_size,self.config.hidden_size)))

...
class Config:
    ...
    def __init__(self, args):
        self.cell = args.cell # for test 2 = rnn
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        labels0 = copy.copy(labels)
        diff = max_length - len(sentence0)
        if diff > 0:
            sentence0 += [zero_vector]*diff
            # nayriz : 
            #+= [zero_vector]*diff is very different than [zero_vector*diff]
            labels0 += [zero_label]*diff   
            pad = [True]*len(sentence) + [False]*diff
        elif diff < 0:
            del sentence0[max_length:]
            del labels0[max_length:]
            pad = [True]*max_length  
        else:             
            pad = [True]*max_length
        
        ret.append( (sentence0,labels0,pad) )        
        ### END YOUR CODE ###  
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embed_index = tf.nn.embedding_lookup(embed_tensor,self.input_placeholder)
        embeddings = tf.reshape(embed_index,(-1,self.max_length,self.config.n_features*self.config.embed_size))
        # WARNING: n_features, not n_word_features!!!
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        if self.config.cell == "rnn":
            ...
            # with __init__(self, input_size, state_size)
        ...
        # should be "None"
        # but you can't put "None" either, because this vector is not fed from the dictionary
        # so the way to do it is to give it the same first dimension as x, which is None
        # when the function is created, and batch_size when fed.        
        h0 = tf.zeros((tf.shape(x)[0],self.config.hidden_size)) 
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
                ...
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):                                    
                        for time_step in range(1,self.max_length):
        ...
        # nayriz : axis =1 because we want a list of feature vectors, not of
        # vectors corresponding to the words themselves
        ### END YOUR CODE
...
if __name__ == "__main__":
    ...
    # NOTE: here "parse" is not related to our own NLP parsing functions, 
    # it's a Python in-built function

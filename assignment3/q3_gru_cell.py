...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            ############
            Uz = tf.get_variable('U_z',shape=(self.input_size,self._state_size),
                                 initializer = weight_initializer)
            Wz = tf.get_variable('W_z',shape=(self._state_size,self._state_size),
                                 initializer = weight_initializer)
            # nayriz : WARNING don't do 
            #bz = tf.get_variable('b_z',initializer = tf.zeros((self._state_size,))) 
            # otherwise you get this type of error:
            # 
            
            bz = tf.get_variable('b_z', shape=(self._state_size), initializer=tf.constant_initializer(0.))            
            ############
            Ur = tf.get_variable('U_r',shape=(self.input_size,self._state_size),
                                 initializer = weight_initializer)
            Wr = tf.get_variable('W_r',shape=(self._state_size,self._state_size),
                                 initializer = weight_initializer)
            #br = tf.get_variable('b_r',initializer = tf.zeros((self._state_size,)))     
            br = tf.get_variable('b_r', shape=(self._state_size), initializer=tf.constant_initializer(0.))            
            ############
            Uo = tf.get_variable('U_o',shape=(self.input_size,self._state_size),
                                 initializer = weight_initializer)
            Wo = tf.get_variable('W_o',shape=(self._state_size,self._state_size),
                                 initializer = weight_initializer)
            #bo = tf.get_variable('b_o',initializer = tf.zeros((self._state_size,)))                          
            bo = tf.get_variable('b_o', shape=(self._state_size), initializer=tf.constant_initializer(0.))             
            ############
            
            zt = tf.sigmoid(tf.matmul(inputs,Uz) + tf.matmul(state,Wz) + bz)
            rt = tf.sigmoid(tf.matmul(inputs,Ur) + tf.matmul(state,Wr) + br)     
            ot = tf.tanh(tf.matmul(inputs,Uo) + rt*tf.matmul(state,Wo) + bo)     
            new_state = zt * state + (1.0 - zt) * ot            
            ### END YOUR CODE ###
